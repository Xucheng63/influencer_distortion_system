...
"""
reclassify_rss.py
读取 RSS/Newsletter 三个 corpus.jsonl（63个账号，1265条帖子）
用五维度新规则 → 负向过滤 → GPT-4o-mini 核实 重新打分。

输入（三个文件合并）：
  research_results/corpus.jsonl          —  8账号  156帖
  research_results_50/corpus_final.jsonl — 34账号  613帖
  research_results_batch2/corpus.jsonl   — 24账号  556帖

运行：
  cd ~/Desktop/ischool/distortion-full-project/distortion-backend
  conda activate ischool
  python reclassify_rss.py

输出（research_results_rss/ 目录下）：
  corpus_v2.jsonl      ← 帖子级，63账号 1265帖，含新分类结果
  profiles_v2.json     ← 账号级汇总
  profiles_v2.csv      ← 汇总表
  report_v2.md         ← 可读报告
...
# ── 路径 ──────────────────────────────────────────────────────────────────────
INPUT_FILES = [
    Path("research_results/corpus.jsonl"),
    Path("research_results_50/corpus_final.jsonl"),
    Path("research_results_batch2/corpus.jsonl"),
]
OUTPUT_DIR    = Path("research_results_rss")
...
async def main():
    ...
    print("\n" + "="*60)
    print("RSS/Newsletter 重新分类 — 五维度 v2")
    print(f"输入: {len(INPUT_FILES)} 个文件合并")
    ...
    handles = {p.get("handle","") for p in deduped}
    print(f"\n合并后: {len(deduped)} 条帖子，{len(handles)} 个账号\n")

    # ── 断点续跑 ───────────────────────────────────────────────────────────────
    done_ids: set = set()
    if PROGRESS_FILE.exists():
        done_ids = set(json.loads(PROGRESS_FILE.read_text()).get("done_ids", []))
    print(f"已完成: {len(done_ids)} 条，剩余: {len(deduped) - len(done_ids)} 条\n")
    ...
    with open(OUT_JSONL, "a", encoding="utf-8") as out_fh:
        for acc_idx, (handle, posts) in enumerate(handle_posts.items(), 1):
            ...
            print(f"  [{acc_idx:3d}/{total_accounts}] {handle:<32} ({len(posts)} 帖)...", end="", flush=True)
            t0 = time.time()
    ...
    for handle, posts in account_posts.items():
        ...
        profiles.append({
            "handle":   handle,
            "platform": posts[0].get("platform", "newsletter"),
    ...
    # ── CSV ────────────────────────────────────────────────────────────────────
    fields = ["handle","platform","category","distortion_index",
    ...
    lines = [
        "# RSS/Newsletter 重分类报告（五维度 v2）",
        f"\n生成时间: {now}",
        "\n公式: `DI = SI×0.30 + AM×0.25 + NC×0.20 + LL×0.15 + TD×0.10`",
        f"\n输入来源: {len(INPUT_FILES)} 个 corpus.jsonl 合并\n",
        "---\n## 概览",
