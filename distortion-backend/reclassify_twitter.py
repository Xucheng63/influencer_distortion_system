#!/usr/bin/env python3
"""
reclassify_twitter.py
读取 research_results_twitter_v2/profiles.json（53个账号，2607条帖子）
用五维度新规则 → 负向过滤 → GPT-4o-mini 核实 重新打分。

分类文本 = post content + comments_text 合并（与之前保持一致）

运行：
  cd ~/Desktop/ischool/distortion-full-project/distortion-backend
  conda activate ischool
  python reclassify_twitter.py

输出（research_results_twitter_v2/ 目录下）：
  profiles_v2.json     ← 账号级，含新 DI + 五维度比率 + posts（带分类结果）
  corpus_v2.jsonl      ← 帖子级，方便跨平台合并
  profiles_v2.csv      ← 汇总表
  report_v2.md         ← 可读报告
...
# ── 路径 ──────────────────────────────────────────────────────────────────────
INPUT_FILE    = Path("research_results_twitter_v2/profiles.json")
...
async def main():
    print("\n" + "="*60)
    print("Twitter v2 重新分类 — 五维度 v2")
    print(f"输入: {INPUT_FILE}")
    print(f"分类文本: tweet content + comments_text 合并")
    ...
    if OUT_JSON.exists():
        ...
        print(f"已完成 {len(result_data)} 个账号，跳过\n")
    ...
    for acc_idx, (handle, prof) in enumerate(raw_data.items(), 1):
        ...
        if not posts:
            print(f"  [{acc_idx:3d}/{total_accounts}] {handle} — 无帖子，跳过")
            continue
        ...
        for post in posts:
            # 合并 tweet + comments_text（与之前保持一致）
            tweet    = post.get("content", "").strip()
            comments = post.get("comments_text", "").strip()
            content  = (tweet + " " + comments).strip()[:800]
        ...
        result_data[handle] = {
            ...
            "total_posts":                n,
            "posts_with_comments":        prof.get("posts_with_comments", 0),
    ...
    all_posts_cnt = sum(len(p.get("posts", [])) for p in profiles)

    print(f"\n{'='*60}")
    print(f"账号={len(profiles)}  帖子={all_posts_cnt}  平均DI={round(statistics.mean(indices),1)}")
    ...
    print(f"\n{'类别':16s} {'n':>3}  {'avgDI':>6}  {'infl%':>6}  {'anx%':>5}  {'LL%':>5}  {'td%':>5}")
    print("-"*58)
    for cat, accs in sorted(by_cat.items(), key=lambda x: -statistics.mean(a["distortion_index"] for a in x[1])):
        ...
        print(f"{cat:16s} {len(accs):3d}  {di:6.1f}  {infl:6.2f}  {anx:5.2f}  {ll:5.2f}  {td:5.2f}")
    ...
    fields = ["handle","platform","category","expected","distortion_index",
              ...
              "total_posts","posts_with_comments","reclassified_at"]
    ...
    lines = [
        "# Twitter/X 重分类报告（五维度 v2）",
        f"\n生成时间: {now}",
        "\n公式: `DI = SI×0.30 + AM×0.25 + NC×0.20 + LL×0.15 + TD×0.10`",
        "\n注：分类文本 = tweet + comments_text 合并；失真归因于账号作者，非评论者。\n",
        "---\n## 概览",
        f"- 账号数: **{len(profiles)}**  帖子数: **{all_posts_cnt}**  平均DI: **{avg}**",
    ...
    print(f"\n✓ profiles_v2.json — {len(profiles)} 个账号")
    print(f"✓ corpus_v2.jsonl  — {all_posts_cnt} 条")
