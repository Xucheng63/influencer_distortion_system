#!/usr/bin/env python3
"""
reclassify_youtube.py
读取 research_results_youtube/corpus.jsonl（32个账号，640条帖子）
用五维度新规则 → 负向过滤 → GPT-4o-mini 核实 重新打分。

运行：
  cd ~/Desktop/ischool/distortion-full-project/distortion-backend
  conda activate ischool
  python reclassify_youtube.py

输出（research_results_youtube/ 目录下）：
...
注意：YouTube content = 视频标题 + 字幕摘要（800字以内），
     信息密度比短帖高，规则精确度更重要，GPT 核实优先去除误报。
"""
...
# ── 路径 ──────────────────────────────────────────────────────────────────────
INPUT_CORPUS  = Path("research_results_youtube/corpus.jsonl")
...
async def main():
    ...
    print(f"读取 {len(all_posts)} 条 (标题+字幕摘要)\n")
    ...
    lines = [
        "# YouTube 重分类报告（五维度 v2）",
        f"\n生成时间: {now}",
        "\n公式: `DI = SI×0.30 + AM×0.25 + NC×0.20 + LL×0.15 + TD×0.10`",
        "\n注：YouTube content = 视频标题 + 字幕摘要，科学教育内容已做专项负向过滤。\n",
        "---\n## 概览",
